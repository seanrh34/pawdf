...
fn model_path(app: &AppHandle) -> PathBuf {
    data_dir(app).join("models").join(MODEL_FILE)
}
...
async fn download(app: &AppHandle, url: &str, dest: &Path, label: &str) -> Result<(), String> {
    ...
    while let Some(chunk) = stream.next().await {
        ...
        app.emit("setup-progress", json!({ "label": label, "got": got, "total": total })).ok();
    }
    drop(file);
...
async fn download_assets(app: AppHandle) -> Result<(), String> {
    if find_server(&app).is_none() {
        ...
        download(&app, &format!("{LLAMA_BASE}{LLAMA_ASSET}"), &archive, "llama.cpp").await?;
        ...
        let out = Command::new(tar)
            .args(["-xf", &archive.to_string_lossy(), "-C", &bin_dir.to_string_lossy()])
            .output()
...
async fn start_llm(app: AppHandle, state: State<'_, Llm>) -> Result<u16, String> {
    ...
    let log = fs::File::create(data_dir(&app).join("llama-server.log")).map_err(|e| e.to_string())?;
    ...
    *state.child.lock().unwrap() = Some(child);
    ...
    // wait for the model to load (health turns 200)
    for _ in 0..600 {
        if health(port).await {
            state.port.store(port, Ordering::SeqCst);
            return Ok(port);
        }
        if let Some(status) = state.child.lock().unwrap().as_mut().and_then(|c| c.try_wait().ok().flatten()) {
            return Err(format!("llama-server exited early ({status}); see llama-server.log in app data"));
        }
...
#[tauri::command]
async fn ask(app: AppHandle, state: State<'_, Llm>, messages: Vec<Value>) -> Result<String, String> {
    let port = state.port.load(Ordering::SeqCst);
    if port == 0 {
...
fn create_session(app: AppHandle, src_path: String) -> Result<Value, String> {
    ...
    let name = src.file_stem().map(|s| s.to_string_lossy().into_owned()).unwrap_or("Document".into());
...
fn get_session(app: AppHandle, id: String) -> Result<Value, String> {
    let dir = session_dir(&app, &id)?;
    let meta: Value = serde_json::from_str(&fs::read_to_string(dir.join("meta.json")).map_err(|e| e.to_string())?)
        .map_err(|e| e.to_string())?;
    let chat: Value = serde_json::from_str(&fs::read_to_string(dir.join("chat.json")).unwrap_or("[]".into()))
...
fn save_chat(app: AppHandle, id: String, chat: Value) -> Result<(), String> {
    fs::write(session_dir(&app, &id)?.join("chat.json"), chat.to_string()).map_err(|e| e.to_string())
}
